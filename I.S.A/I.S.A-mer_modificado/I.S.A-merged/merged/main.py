...
def desenhar_pergunta(pergunta):
    screen.fill((0, 0, 0))
    fonte_quiz = carregar_fonte("upheavtt.ttf", sf(18))  # ← fonte menor
    header = fonte_titulo.render(
        f"{pergunta['disciplina']}  |  Nivel: {pergunta['nivel']}", True, constants.YELLOW)
    screen.blit(header, (sx(50), sy(30)))

    textos = [
        pergunta["pergunta"],
        "A) " + pergunta.get("opcao_a", pergunta.get("Opcao_a", "")),
        "B) " + pergunta.get("opcao_b", pergunta.get("Opcao_b", "")),
        "C) " + pergunta.get("opcao_c", pergunta.get("Opcao_c", "")),
        "D) " + pergunta.get("opcao_d", pergunta.get("Opcao_d", "")),
        "E) " + pergunta.get("opcao_e", pergunta.get("Opcao_e", "")),
    ]

    y = sy(80)  # ← começa mais cedo
    for texto in textos:
        palavras = texto.split(" ")
        linha = ""
        for palavra in palavras:
            teste = linha + palavra + " "
            if fonte_quiz.size(teste)[0] > constants.SCREEN_WIDTH - sx(100):
                surf = fonte_quiz.render(linha.strip(), True, constants.WHITE)
                screen.blit(surf, (sx(50), y))
                y += sy(22)  # ← espaçamento menor entre linhas quebradas
                linha = palavra + " "
            else:
                linha = teste
        if linha.strip():
            surf = fonte_quiz.render(linha.strip(), True, constants.WHITE)
            screen.blit(surf, (sx(50), y))
            y += sy(30)  # ← espaçamento menor entre opções
        ...
        for event in pygame.event.get():
            ...
            if estado_jogo == "INTRO":
                ...
                if resposta and indice < len(perguntas):
                    gabarito = perguntas[indice].get("resposta", "").strip().lower().rstrip(";")
                    print(f"resposta: '{resposta}' | gabarito: '{gabarito}'")
